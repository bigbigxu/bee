...
class CoreMysql
{
	...
	public function findById($ids)
	{
		...
		if($num == 1) {
			$res= $this
                ->where("{$this->fields[$this->tableName]['pk']}=:id")
                ->params(array(":id" => $ids))
                ->one();
		} else {
			//如果是字符串主键，要加引号
			$tmp = explode(',', $ids);
			$tmp = array_map(array($this, 'addQuot'), $tmp);
			$ids = implode(',', $tmp);
			$res = $this
                ->where("{$this->fields[$this->tableName]['pk']} in ({$ids})")
                ->all();
		}
		return $res;
	}
    ...
    public function findByAttr($attr, $num = 1)
    {
        ...
        return $this->where($where)->params($params)->$method();
    }
	...
	public function update($data, $where = '', $params = array())
	{
		...
		foreach($data as $key => $value) {
            ...
            $bindName = ":{$key}";
            if(isset($this->sqlQuery['params'][$bindName])) {
                $bindName = ":{$this->autoBindPrefix}{$key}";
            }
    ...
    public function updateById($data, $id)
    {
        ...
        $bindName = ":{$field}";
        $where = "`{$field}`={$bindName}";
        $params = array($bindName => $id);
        return $this->update($data, $where, $params);
    }
     ...
     * @param string $value 值 like 并不会自加上%
    ...
    public function addFilter($field, $op, $value, $type)
    {
        $bindName = ":{$field}";
        if(isset($this->sqlQuery['params'][$bindName])) {
            $bindName = ":{$this->autoBindPrefix}{$field}";
        }
        $this->sqlQuery['where'] .= " {$type} {$field} {$op} {$bindName} ";
        $this->sqlQuery['params'][$bindName] = $value;
        return $this;
    }
